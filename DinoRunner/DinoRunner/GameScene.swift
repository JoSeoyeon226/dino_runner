...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //record
    var score_storage = 0 as Int
    var recordNode: SKLabelNode!
    ...
    override func didMove(to view: SKView) {
        ...
        //이전기록
//        record = 0
        recordNode = SKLabelNode(fontNamed: "Arial")
        recordNode.fontSize = 15
        recordNode.zPosition = foreground
        recordNode.text = "이전기록: 0"
        recordNode.fontColor = SKColor.gray
        recordNode.position = CGPoint(x: 820, y: 720)
...
//        scoreNode.fontSize = 30
        ...
        scoreNode.text = "현재기록: 0"
        scoreNode.fontColor = SKColor.gray
        scoreNode.position = CGPoint(x: 820, y: 700)
//        scoreNode.position = CGPoint(x: 300, y: 150)
        
        //reset instructions
        resetInstructions = SKLabelNode(fontNamed: "Arial")
        resetInstructions.fontSize = 30
//        resetInstructions.zPosition = foreground
        resetInstructions.text = "♥︎디순이를 위해 다시 시작♥︎"
        resetInstructions.fontColor = SKColor.white
//        resetInstructions.position = CGPoint(x: 665, y: 500)
        resetInstructions.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
        
        //new record
        new_record = SKLabelNode(fontNamed: "Arial")
        new_record.fontSize = 50
        new_record.zPosition = foreground
        new_record.text = "♥︎!!신기록!!♥︎"
        new_record.fontColor = SKColor.white
        new_record.position = CGPoint(x: 665, y: 420)
        ...
        gameNode.addChild(recordNode)
        gameNode.addChild(resetInstructions)
        gameNode.addChild(new_record)
        self.addChild(gameNode)
    }
    
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if(gameNode.speed < 1.0){
            UserDefaults.standard.set("\(score)", forKey: "record_score")
            new_record.fontColor = SKColor.purple
            resetGame()
            return
        }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if(gameNode.speed > 0){
            ...
            score += 1
            scoreNode.text = "현재기록: \(score/5)"
            
            score_storage =  UserDefaults.standard.integer(forKey: "record_score")
            recordNode.text = "이전기록: \(score_storage/5)"
    ...
    func resetGame() {
        ...
        dinoSprite.position = CGPoint(x: self.frame.size.width * 0.38, y: dinoYPosition!)
    ...
    func createDinosaur() {
        ...
        dinoSprite.position = CGPoint(x: screenWidth * 0.38, y: dinoYPosition!)
        dinoSprite.run(SKAction.repeatForever(runningAnimation))
    }
